...
PERIODS = [
    ("پایه", "۶۹۰٬۰۰۰", "تعیین نشده", "تعیین نشده"),
    ("استاندارد", "۱٬۲۹۰٬۰۰۰", "تعیین نشده", "تعیین نشده"),
    ("پرو", "۲٬۴۹۰٬۰۰۰", "تعیین نشده", "تعیین نشده"),
]
period_rows = "\n".join(
    f'<tr><td class="feat">{n}</td><td class="c">{p}</td>'
    f'<td class="c"><span class="chip tbd">تعیین نشده</span></td>'
    f'<td class="c"><span class="chip tbd">تعیین نشده</span></td></tr>'
...
DECISIONS = [
    "قیمت نهایی اشتراک‌های سه‌ماهه",
    "قیمت نهایی اشتراک‌های شش‌ماهه",
    "میزان تخفیف دوره سه‌ماهه",
    "میزان تخفیف دوره شش‌ماهه",
    "بررسی نهایی اقتصاد افزونه‌ها در برابر Upgrade پلن",
    "تعیین قیمت بسته‌های شارژ اعتبار پیامک",
]
decisions = "\n".join(f'<li><span class="dnum">{i+1}</span>{d}</li>' for i, d in enumerate(DECISIONS))

# ---------------- policies (sections 22-24) ----------------
POLICIES = [
    ("سیاست اعتبار پیامک", "Pay As You Use",
     "اعتبار SMS مستقل از قیمت پلن یا باشگاه مشتریان است؛ کاربر متناسب با مصرف، اعتبار پیامکی حسابش را شارژ می‌کند. پلن پرو یا خرید باشگاه به معنی پیامک نامحدود نیست و هزینه ارسال واقعی از اعتبار شارژشده کسر می‌شود.",
     ["پلن پرو = پیامک نامحدود نیست", "خرید باشگاه = اعتبار پیامک نامحدود نیست", "هزینه ارسال از اعتبار شارژشده کسر می‌شود"]),
...
<div class="wrap">
  ...
  <p class="note">قیمت ماهانه پلن‌ها تأیید شده و مبنای فعلی قیمت‌گذاری منوچ هستند؛ دوره‌های سه‌ماهه و شش‌ماهه هنوز تعیین نشده‌اند.</p>
</div>
...
<section class="wrap sec">
  ...
  <div class="plug-grid">
    <div class="tbl">
      <table class="plug">
        <thead><tr><th class="feat">افزونه ساختاری</th><th>قیمت ماهانه فعلی</th></tr></thead>
        <tbody>
{plug_rows}
        </tbody>
      </table>
